...
class SBERT_vs_OpenAI_evaluator():
    ...
    def evaluate_SBERT_vs_OpenAI_results(self, html_file):
        ...
        # json_string = "{\"winner\": \"<either 'SBert' or 'OpenAI'>\", \"rationale\": \"<detailed explanation of why the chosen column is better>\"}"
        prompt = (f"Perform a deep analysis of the following html. It contains a table with "
                  f"two columns of data."
                  f"Each column contains text fragments from querying a large corpus of "
                  f"information. "
                  f"The two columns represent records generated by two different algorithms. "
                  f"Ignore blank cells, and do not consider the raw number of rows returned as a "
                  f"measure of relevance."
                  f"Only base your evaluation on which algorithms created a set fragments with "
                  f"the best "
                  f"detailed and most relevant "
                  f"information for use in creating a summary of the topic "
                  f"contained in the first header of the HTML document."
                  f"For your response, return the winner as either 'One' if the first column is "
                  f"superior or 'Two' if the second column is superior.  Provide the winner "
                  f"as "
                  f"the first line with nothing else on it, followed by a detailed "
                  f"explanation of your analysis\n"
                  f"Here is the HTML document:\n"
                  f" {html_text} \n\n")
